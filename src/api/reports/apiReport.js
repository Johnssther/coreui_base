...
class ApiReport {
    constructor() {
     ...
     getGastosReport(data) {
        return fetch(`${this.URL}gastosreport?api_token=${this.API_TOKEN}&user_id=${JSON.parse(localStorage.getItem('auth')).id}&filterDate_in=${data.filterDate_in}&filterDate_out=${data.filterDate_out}`)
